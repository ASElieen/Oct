...
export class UserCache extends BaseCache {
  ...
  public async getRandomUserFromCache(userId: string, excludedUsername: string): Promise<IUserDocument[]> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect()
      }
      ...
      for (const key of randomUsers) {
        ...
        //如果存在则说明是当前用户的follower 跳过
        //不是当前用户follower才会放入replies
        if (followerIndex < 0) {
      ...
      log.error(error)
      throw new ServerError('获取随机user时出现错误,请重试')
    }
  }
}